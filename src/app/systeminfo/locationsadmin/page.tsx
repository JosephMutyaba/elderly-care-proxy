...
import {getLocationsAdmin, getLocationsAdminCount} from "@/app/actions/locationactions";
import PaginatedLocationDataTable from "@/app/customcomponents/PaginatedLocationDataTable";


export default function LocationsPageOrGPS() {
    const [currentPage, setCurrentPage] = useState<number>(1);
    const [totalRecords, setTotalRecords] = useState<number | null>(1);
    const [gpsValues, setGpsValues] = useState<Tables<'locationdata'>[] | null>(null);
    const pageSize = 10;

    // Fetch devices from Supabase
    const fetchMotionRecords = async () => {
        const { data, count, error } = await getLocationsAdmin(currentPage, pageSize);
        const { count: countTotal, error:countError } = await getLocationsAdminCount();
    ...
    return (
        <div className="container mx-auto py-8">
            ...
            <PaginatedLocationDataTable
                locationRecords={gpsValues}
                pageSize={pageSize}
